  ...
  return (
    <section id="process" className="container-page scroll-mt-20 py-20 sm:py-32">
      ...
      {/* Mobile: pilha "sticky stack" -- cada card gruda (position: sticky)
          ...
          vazio no meio da rolagem. A margem-inferior grande (mobile only) é
          a "pista" que segura o card no lugar enquanto o próximo se
          aproxima por baixo. No desktop (md+) volta pra grade normal lado a
          lado, sem sticky nem margem extra. */}
      <div className="mt-10 md:grid md:gap-4 md:grid-cols-2 lg:grid-cols-4">
        {PROCESS.map((s, i) => (
          <StackCard
            ...
            zIndex={i + 1}
            delay={i * 90}
            className="sticky mb-[45vh] flex min-h-[13rem] flex-col justify-center rounded-2xl border border-line/10 glass p-7 shadow-2xl shadow-bg/60 md:static md:mb-0 md:min-h-0 md:justify-start md:shadow-none"
          >
