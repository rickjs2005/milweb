 ...
 * clique em link de WhatsApp (wa.me) ou de e-mail (mailto:) e registra no Vercel Analytics de
...
export function TrackConversions() {
  ...
  useEffect(() => {
    const onClick = (e: MouseEvent) => {
      const link = (e.target as HTMLElement).closest<HTMLAnchorElement>('a[href*="wa.me"], a[href^="mailto:"]');
      if (!link) return;
      const source =
        link.closest("[id]")?.id ||
        (link.closest("header") ? "nav" : link.closest("footer") ? "footer" : "page");
      const event = link.getAttribute("href")?.startsWith("mailto:") ? "email_click" : "whatsapp_click";
      track(event, { source, path: pathname ?? "/" });
    };
