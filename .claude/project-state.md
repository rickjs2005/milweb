...
- **Mouse** (`GLOBE_POINTER`, `HeroGlobe.tsx`): só com ponteiro fino e ≥ 768 px. A posição do cursor na janela vira alvo em [-1, 1]; o valor que o shader recebe persegue o alvo no loop com amortecimento por tempo (`k = 1 − (1 − 0,07)^(dt·60)`), somado a `uSpin` (±0,52 rad) e `uTilt` (±0,16 rad). Escala pelo `f.mark`: antes da esfera formada o mouse não manda nada, a transformação do "O" continua só do scroll. `pointerleave` no `<html>` zera o alvo → volta ao repouso sozinho. Custo zero: o loop já era contínuo com o globo em cena. Mobile: nada muda.
- **Vida** (`GLOBE_LIFE`, `globe-frag.ts`): pulso no marcador (anel que nasce em 0,05 rad, cresce até 0,29 e apaga com (1−ph)², período 2,4 s) e **cinco arcos de grande círculo** do Brasil para Nova York, Lisboa, Berlim, Cidade do México e Buenos Aires, em tinta a 35 %, que se **desenham a partir do Brasil** conforme `uMark` acende, cada um com um ponto Signal Green viajando (slerp no tempo, velocidades 0,08–0,16 ciclos/s, fases distintas). O renderer não tem uniform de array — os vetores B, as normais N = A×B e os comprimentos L são **constantes cozidas no GLSL em build** a partir do config (blocos desenrolados, um por arco). Único uniform novo: `uTime`. Distância ao arco = |dot(P, N)|; trecho válido = lente onde o pixel está a menos de L de A e de B. Os pontos ficaram menores que o marcador de origem (0,022 rad contra 0,030) depois do primeiro screenshot: do mesmo tamanho, competiam com o Brasil.
- **QA** (`scripts/.rsc-audit/globe/pointer.mjs`, não versionado): janela real 1920×1080 com `?quality=high`, leva o Hero a 92 % do pin, move o mouse para centro/esquerda/direita/topo/base, lê `__mwGlobe.pointer` (agora exposto), recorta o globo e prova o retorno ao repouso; 30 fps com mouse parado, console limpo; 390×844 (touch) só arcos e pulso, 30,5 fps. Olhei os cinco recortes: o Brasil fica visível em todas as posições e o globo "rola" na direção do cursor. `tsc`/`lint`/`build` verdes (`/[lang]` 201 kB first load; o shader continua em import dinâmico).

**Armadilhas do QA (valem para todos os scripts do globo):**
- `window.scrollTo` **não move a página** nesta build (Lenis + `html, body { overflow-x: clip }`): os scripts antigos (`hero-frames.mjs`) que usam `scrollTo` hoje ficam em `scrollY = 2`. Rolar **pela roda** (`page.mouse.wheel`) funciona; o probe novo corrige em passos até o alvo. Não mandar `wheel` DEPOIS de um `scrollTo`: o Lenis trata o delta como alvo e volta ao topo.
...
Fase 11 — Analytics e conversão ✓  Web Analytics ativado pelo Rick em 05/09; clique de WhatsApp em produção gerou POST 200 em /_vercel/insights/event (whatsapp_click, source=contact); email_click no mesmo listener
